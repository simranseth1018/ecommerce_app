...
export const getTokenStatus = (token: LicenseToken | null): Status => {
  ...
  if (tokenExpired(token)) {
    return {
      state: 'error',
      status: 'Expired',
      title: t('licensing.get-token-status.title.token-expired', 'Token expired'),
  ...
  if (tokenWillExpireSoon(token)) {
    return {
      ...
      title: t('licensing.get-token-status.title.token-expires-soon', 'Token expires soon'),
...
export const calculateActiveUsers = (stats: ActiveUserStats | null | undefined, token: LicenseToken | null): number => {
...
export const getTrialStatus = (token: LicenseToken | null): Status => {
  ...
  if (trialExpired(token)) {
    return {
      state: 'error',
      status: 'Expired',
      title: t('licensing.get-trial-status.title.trial-expired', 'Trial expired'),
  ...
  return {
    ...
    title: t('licensing.get-trial-status.title.trial-active', 'Trial active'),
