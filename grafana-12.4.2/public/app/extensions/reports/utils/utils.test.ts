...
import { applyDefaultVariables, toReportVariables } from './variables';
...
describe('Report variable utils', () => {
  describe('toReportVariables', () => {
    ...
    it('should return undefined if no variables are provided', () => {
      expect(toReportVariables(undefined)).toEqual({});
    });
    it('should return undefined for an empty variable object', () => {
      expect(toReportVariables({} as any)).toEqual({});
    });

    it('should return an object with values as string array and filter out keys with empty values', () => {
      expect(toReportVariables(variables as any)).toEqual({
        custom: ['1', '2'],
        datacenter: ['A'],
    ...
    it('should handle errors when getting value for URL and return empty array', () => {
      ...
      // Mock getValueForUrl to throw an error
      const adapter = variableAdapters.get('textbox');
      const originalGetValueForUrl = adapter.getValueForUrl;
      adapter.getValueForUrl = jest.fn().mockImplementation(() => {
        throw new Error('Failed to get value for URL');
      });

      const result = toReportVariables([errorVariable] as any);

      expect(result).toEqual({
        errorVar: [],
      });
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        'Error getting value for variable %s:',
        'errorVar',
        expect.any(Error)
      );

      // Restore original implementation
      adapter.getValueForUrl = originalGetValueForUrl;
